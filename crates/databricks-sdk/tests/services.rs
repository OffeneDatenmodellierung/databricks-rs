...
use databricks_sdk::service::compute::{
    ClusterSource, ListClustersFilterBy, ListClustersRequest, ListClustersSortBy,
    ListClustersSortByField, State,
};
use databricks_sdk::service::jobs::{
    GetRunRequest, QueueSettings, RunLifeCycleState, RunNow, RunResultState,
};
use databricks_sdk::service::provisioning::WorkspaceStatus;
use databricks_sdk::{AccountClient, Config, Error, WorkspaceClient};
...
fn every_known_enum_value_round_trips() {
    use databricks_sdk::service::compute::{DataSecurityMode, ListClustersSortByDirection};
    use databricks_sdk::service::jobs::PerformanceTarget;
    macro_rules! check {
        ($($t:ident),+) => {$(
            for s in $t::KNOWN {
                let v = $t::from(*s);
...
async fn clients_accept_a_custom_credential_chain() {
    use databricks_sdk::core::auth::{DefaultCredentials, PatCredentials};
    let server = MockServer::start().await;
