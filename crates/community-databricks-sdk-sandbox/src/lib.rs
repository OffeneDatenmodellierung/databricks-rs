...
//! (`community_databricks_sdk::service::sandbox`).
...
impl SandboxApi {
    ...
    ) -> ::community_databricks_core::Result<Sandbox> {
    ...
    ) -> ::community_databricks_core::Result<ExecuteCommandSyncResponse> {
        let path = format!(
    ...
    ) -> ::community_databricks_core::Result<ListSandboxesResponse> {
        let path = String::from("/api/2.0/sandboxes");
        let mut call = Call::new(Method::GET, path).workspace();
    ...
    pub async fn list_sandboxes_all(
        &self,
        request: ListSandboxesRequest,
    ) -> ::community_databricks_core::Result<Vec<Sandbox>> {
